...
<script>
    $(window).on('load', function(){
        ...
        if ($(window).width() > 1024) {
            ...
            var headerHeightpx = headerHeight + 26 + 'px';
            //alert(headerHeightpx);
            $('.fwidht-sidebar').css('top',headerHeightpx);  
            $('.mw-live-edit .fwidht-sidebar').css('top', headerHeight+64+'px'); 
            $(document).scroll(function() {
                var scrollDistance = $(document).scrollTop(); 
                if ( scrollDistance >= 0 ) {
                    $('.fwidht-sidebar').css('top',headerHeightpx);
                    $('.mw-live-edit .fwidht-sidebar').css('top', headerHeight+56+'px'); 
                } 
                let j = 2;
                for(let i = 10; i <=60 ; i+=10){
                    if ( scrollDistance >= i ) { 
                        $('.fwidht-sidebar').css('top',headerHeight - j + 'px');
                        $('.mw-live-edit .fwidht-sidebar').css('top',headerHeight - j +51+ 'px'); 
                    }
                    j+=10;
                }

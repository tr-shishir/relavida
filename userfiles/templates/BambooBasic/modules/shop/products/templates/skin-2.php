...
*/
?>
<link rel="stylesheet" href="<?php print template_url(); ?>modules/shop/product_style.css" type="text/css"/>
<?php
...
<?php if (!empty($data) && !isset($_GET['slug']) && !isset($_GET['wishlist_id'])):
    ...
    <div class="row shop-products">
        <?php foreach ($data as $item) :
            ...
            <div class="col-12 col-md-6 col-lg-3 col-xl-3 item-<?php print $item['content_id'] ?>" data-masonry-filter="<?php //print $itemCats; ?>" itemscope itemtype="">
                <div class="product">
                    <input type="hidden" name="price" value="<?php print $item['price'] ?>" />
                    <input type="hidden" name="content_id" value="<?php print $item['content_id'] ?>" />
                    <input type="hidden" name="tax_rate" value="<?php print $taxrate ?>" />
                            ...
                            <div class="hover">
                                <?php if ($show_fields == false or in_array('add_to_cart', $show_fields)):?>
                                    <?php if (!isset($in_stock) or $in_stock == false) : ?>
                                        <button title="<?php _e('Out Of Stock'); ?>" disabled="disabled" class="btn btn-primary product-cart-icon"><span class="material-icons">remove_shopping_cart</span></button>
                                ...
                                <?php //if ($show_fields == false or ($show_fields != false and in_array('read_more', $show_fields))) : ?>
                                    <a href="<?php print url($item['url']); ?>" title="<?php _e('View'); ?>" class="btn btn-default"><i class="material-icons">remove_red_eye</i></a>
                                <?php //endif; ?>
                    ...
                    <?php if ($is_logged) : ?>
                        <?php if (get_option('enable_wishlist', 'shop')) : ?>
                            <span class="material-icons cust-btn wishlist-btn-<?= $item['content_id']; ?>" id="">
                                favorite_border
                            </span>

                            <div class="toggle-content wishlist-content-<?= $item['content_id']; ?>" id="">
                                ...
                                <script>
                                    $(document).ready(function() {
                                        $(".wishlist-btn-<?= $item['content_id']; ?>").click(function() {
                                            $(this).parent().toggleClass("wishlistToggled");
                                            $(".wishlistToggled .wishlist-content-<?= $item['content_id']; ?>").animate({
                                                width: "toggle"
                                            });
                                        });
                                    });

                    ...
                    <?php if ($show_fields == false or in_array('add_to_cart', $show_fields)): ?>
                        <div class="add-to-cart">
                            ...
                            <?php elseif(isset($item['price']) && (floatval($item['price']) > 0)): ?>
                                <button title="in den Warenkorb" class="btn btn-success product-cart-icon" type="button" onclick="<?php if(isset(mw()->user_manager->session_get('bundle_product_checkout')[0]) && $update_global_bundle_discount_condition == 0) { ?>carttoggole('<?php print $item['content_id'] ?>');<?php }else{ ?>carttoggolee('<?php print $item['content_id'] ?>');<?php } ?>">

...
<?php 
    ...
    if(url_segment(0) == 'shop' and url_segment(1)){
        $categories = DB::table('categories')->where('url', url_segment(1))->where('is_hidden', 0)->where('status', 1)->select('id')->first();
        if(isset($categories)){
            $query = $query->leftJoin('categories_items',function($joining){
                                $joining->on('group_product.product_id', '=', 'categories_items.rel_id');
                        })
                        ->where('categories_items.parent_id' , $categories->id)
        ...
        if(isset($settings['tags']) and !empty($settings['tags']->value)){
            ...
            $tags = explode(',', $tags);
            $query = $query->leftJoin('tagging_tagged',function($joining){
                            $joining->on('group_product.product_id', '=', 'tagging_tagged.taggable_id');
                        })
                        ->whereIn('tagging_tagged.tag_name' , $tags);
        }
                    ...
                    <div class="product-sample-inner">
                        ...
                        <div class="product-sample-hover-bottom">
                            ...
                            <?php else: ?>
                                ...
                                <a href="javascript:;" onclick="<?php if(isset(mw()->user_manager->session_get('bundle_product_checkout')[0]) && $check_bundle_discount_condition == 0) { ?>carttoggole('.shop-products .item-<?php print $item['content_id'] ?>');<?php }else{ ?>carttoggolee('.shop-products .item-<?php print $item['content_id'] ?>');<?php } ?>" class="btn btn-primary product-cart-icon"><i class="material-icons ">shopping_cart</i> in den Warenkorb</a>
                            <?php endif; ?>
                        </div>
                    </div>

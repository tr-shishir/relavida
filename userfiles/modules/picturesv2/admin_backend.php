...
$rand = uniqid();
if ($for_id != false) {
    $media = DB::table('product_media')->where('rel_id',$for_id)->get();
} else {
    $sid = mw()->user_manager->session_id();
    $media = DB::table('product_media')->where('rel_id',0)->get();
}
?>
...
<script>
    after_upld = function (a, e, f, id, module_id) {
        if (e !== 'done') {
            ...
            mw.module_pictures_v2.after_upload(data);
        }
        if (e === 'done') {
            setTimeout(function () {
                ...
                if (self !== top && typeof parent.mw === 'object') {
                    parent.mw.reload_module('picturesv2');
                    ...
                    if (self !== top && typeof parent.mw === 'object') {
                        ...
                        parent.mw.reload_module('content', function () {
                            mw.reload_module('#' + module_id);
                            parent.mw.reload_module('picpicturesv2tures');
                        });
                    }
                }

                $('.admin-thumb-item-loading').remove();
                $('[data-type="picturesv2/admin"]').trigger('change')
...
<script>
    ...
    var getMediaImage = function () {
        var dialog = mw.top().tools.moduleFrame('picturesv2/media_library');
...
<div class="left m-t-20" id="admin-thumbs-holder-sort-<?php print $rand; ?>">
    ...
    <div class="admin-thumbs-holder">
        <?php if ($for_id != false) : ?>
            <module type="picturesv2/admin_backend_sortable_pics_list" for="<?php print $for ?>" for_id="<?php print $for_id ?>"/>
        <?php else: ?>
            <module type="picturesv2/admin_backend_sortable_pics_list" for="<?php print $for ?>" session_id="<?php print $sid ?>"/>
        <?php endif; ?>
    </div>

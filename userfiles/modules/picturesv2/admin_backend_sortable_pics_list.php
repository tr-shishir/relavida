...
$media = false;
if ($for_id != false) {
    $media = DB::table('product_media')->where('rel_id',$for_id)->get()->toArray();
} else {
    $sid = mw()->user_manager->session_id();
    $media = DB::table('product_media')->where('rel_id',0)->get()->toArray();
}
...
<script>
    $(document).ready(function () {
        mw.module_pictures_v2.init('#admin-thumbs-holder-sort-<?php print $rand; ?>');
        ...
        mw.$('#mw-admin-post-media-type')
            ...
            .on('changed.bs.select', function () {
                if ($(this).selectpicker('val') == "instagram") {
                    // console.log('there')
                    return false;
                }
    ...
    <?php if (is_array($media)): ?>
        ...
        <?php foreach ($media as $key => $item): 
            ...
            <div class="admin-thumb-item admin-thumb-item-<?php print $item['id'] ?>"
                ...
                if(!remote_file_exists($tn)){
                    $tn = thumbnail($item['webp_image'], 200, 200, true) ?? thumbnail($item['filename'], 200, 200, true);
                }?>
                ...
                <span class="mdi mdi-delete image-settings remove-image tip" data-tip="Delete Image"
                      onclick="mw.module_pictures_v2.del('<?php print $item['id'] ?>');"></span>
        ...
        <div class="admin-thumb-item-uploader-holder">
            <div class="dropable-zone small-zone square-zone">
                <div class="holder">
                    <button type="button" class="btn btn-link"><?php _e('Add file or drop file to upload'); ?></button>
                </div>
            </div>

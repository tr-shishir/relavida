...
mw.controlFields = {
    ...
    description: function (conf) {
        ...
        return this._wrap(
            this._label(conf),
            this._description(conf),
            '<textarea '+placeholder + '  ' + id + ' ' + name + ' ' + required + ' class="mw-ui-field w100"></textarea>'
        );
    },
    ...
    var UIFormControllers = {
        ...
        layout: function (options) {
            ...
            $.each(layoutsData, function(){
                ...
                $(li).find('input').on('click', function(){
                    ...
                    scope.link = mw.top().win.location.href.split('#')[0] + '#' + el.id;
        ...
        page: function (options) {
            ...
            var defaults = {
                ...
                description: {
                    label: mw.lang('Link description'),
                    description: mw.lang('Selected text for the link.'),
                },
                target: {
                    label: mw.lang('Open the link in a new window')
            ...
            this.settings = options;
            if (options.text === true) options.text = defaults.text;
            if (options.description === true) options.description = defaults.description;
            if (options.target === true) options.target = defaults.target;

            var root = document.createElement('div');
            root.className = 'mw-ui-form-controller-root';
            var _linkText = '', _linkDescription = '', _linkUrl = '', _target = '';
            UIFormControllers._title(this.settings, root)
            var treeEl = document.createElement('div');
            ...
            if (options.description) {
                _linkDescription = mw.controlFields.description({
            ...
            var holder = document.createElement('div');
            holder.append(_linkText);
            holder.append(_linkDescription);
            holder.append(treeEl);
            holder.append(_target);


            var textField = holder.querySelector('[name="text"]');
            var textDescription = holder.querySelector('[name="description"]');
            var targetField = holder.querySelector('[name="target"]');
            ...
            this.getValue = function () {
                var val = {};
                if(textField) val.text = textField.value;
                if(textDescription) val.description = textDescription.value;
            ...
            this.setValue = function (val) {
                val = val || {};
                if(textField) textField.value = val.text || '';
                if(textDescription) textDescription.value = val.description || '';
                if(targetField) targetField.checked = val.target;
                return val;
            };
        ...
        url: function (options) {
            ...
            var defaults = {
                text: {
                    label: mw.lang('Beschreibung des Menutitel'),
                    description: mw.lang('Dieser Name trägt dein Menupunkt.'),
                },
                link: {
                    label: mw.lang('<a id="website_url" target="_blank">Wähle eine Ziel-URL</a>'),
                    description: mw.lang('Das Menu wird mit dieser Seite verlinkt'),
                    placeholder: "http://",
                },
                description: {
                    label: mw.lang('Kurzbeschreibung'),
                    description: mw.lang('Dieser Hinweistext erscheint unterhalb des Menu und ist optinal.'),
                },
                target: {
                    label: mw.lang('Open the link in a new window')
                },
            ...
            var root = document.createElement('div');
            root.className = 'mw-ui-form-controller-root';
            var _linkText = '', _linkUrl = '', _linkDescription = '', _target = '';
            UIFormControllers._title(this.settings, root)
            if (options.text) {
                _linkText = mw.controlFields.field({
                    label: options.text.label,

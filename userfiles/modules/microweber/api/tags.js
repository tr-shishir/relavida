    ...
    */
    ...
    this.rend = function(){
        $.each(this.options.data, function(i){
            ...
            if(data.type != 'page' && data.title != 'Shop'){
         ...
         if(this.options.rounded){
            ...
            if(config.close){
                tag_close.onclick = function () {
                    if(this.parentElement.parentElement.parentElement.dataset.parent == 'shop'){
                        if(this.parentElement.getElementsByClassName("tag-label-content")[0].innerHTML != "Shop"){
                            let firstIndex = $(".btn.btn-sm.mb-2.mr-2.btn-outline-primary").data('index');
                            if($(".post-category-tags").find(`[data-index='2']`).length==0 && this._index == firstIndex){                                let index = this._index;
                            ...
                            let firstIndex = $(".btn.btn-sm.mb-2.mr-2.btn-outline-primary").data('index');
                            if($(".post-category-tags").find(`[data-index='2']`).length==0 && this._index == firstIndex+1){                                let index = this._index;
                                mw.tools.confirm("Wenn du diese Kategorie entfernst dann wird das Produkt den Status unvollständig erhalten. Wenn du die Kategorie ändern möchtest, solltest du zuerst die neue zuweisen. So vermeidest du einen 'unvollständigen Status'. Nachdem chdem die neue Kategorie zugewiesen wurde, kannst du die alte (nicht mehr benötigte) entfernen.", function () {
                                    scope.removeTag(index);
                                });
                            }else{

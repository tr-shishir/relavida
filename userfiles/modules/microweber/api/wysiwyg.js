...
mw.wysiwyg = {
    ...
    media: function (action) {
        ...
        if (mw.wysiwyg.isSelectionEditable() || mw.$(mw.target.item).hasClass("image_change") || mw.$(mw.target.item.parentNode).hasClass("image_change") || mw.target.item === mw.image_resizer) {
            mw.wysiwyg.save_selection();
            var dialog;
            var handleResult = async function (res) {
                let url = res.src ? res.src : res;
                let rawurl = url;
                const response = await $.post(window.base_url+'/api/v1/compress_url', { url: url }, ress => ress.url)
                if(response.success) {
                    url = response.url
                }
                if(action === 'editimage') {
                    if(mw.image.currentResizing) {
                        if (mw.image.currentResizing[0].nodeName === 'IMG') {
                            ...
                            mw.image.currentResizing.attr("data-raw-url", rawurl);
                            mw.image.currentResizing.addClass("rawurlsrc");
                            mw.image.currentResizing.css('height', 'auto');
                        }
                        else {
                            mw.image.currentResizing.css("backgroundImage", 'url(' + mw.files.safeFilename(url) + ')');
                            mw.image.currentResizing.addClass("rawurlstyle");
                            mw.image.currentResizing.attr("data-raw-url", rawurl);
                            if(parent.mw.image.currentResizing) {
                                mw.wysiwyg.bgQuotesFix(parent.mw.image.currentResizing[0])
                            }
    ...
    insertMedia: function (url, type) {
        ...
        if(!type) {
            if(['png','webp','gif','jpg','jpeg','tiff','bmp','svg'].indexOf(ext) !== -1) {
                type = 'image';
            }

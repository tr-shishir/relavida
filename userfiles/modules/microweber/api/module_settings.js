mw.moduleSettings = function(options){
    ...
    */
    ...
    this.createItemHolder = function(i){
        ...
        holder.className = 'mw-ui-box mw-module-settings-box mwSetting_singleBox';
        ...
        // var ddd = $('#settings-box').find('.mw-module-settings-box').length;
    ...
    $( document ).ready(function() {
        ...
        // $('.mwSetting_box1').children('.mw-ui-box-content').children('.prop-ui-field-holder').find('input').val();
        // $('.mwSetting_box1').children('.mw-ui-box-header ').find('.questionName').text('sdddd');

        $("#settings-box").children('.mwSetting_singleBox').each(function(e) {
            var getQuestionName = $(this).children('.mw-ui-box-content').children('.prop-ui-field-holder').find('input').val();
            var getQuestionName1 = $(this).children('.mw-ui-box-content').children('.prop-ui-field-holder:nth-child(3)').find('input').attr('value');
            var getQuestionName2 = $(this).children('.mw-ui-box-content').children('.prop-ui-field-holder:nth-child(4)').find('input').attr('value');

            if (getQuestionName1 != options.page_id) {
                $(this).hide();
            }
            if (getQuestionName2 != options.moduleId) {
                $(this).hide();
            }
            if(getQuestionName == ''){
                $(this).children('.mw-ui-box-header ').find('.questionName').text('Question Name');
            }else{
                $(this).children('.mw-ui-box-header ').find('.questionName').text(getQuestionName);
            }
        });


        $('.prop-ui-field').click(function() {
            if ($(this).parentsUntil(".mw-dialog-iframe-autoheight").hasClass("active")) {
                $(this).addClass('TypeQuestion');
                $(this).on('keydown, keyup',function() {
                    $(this).parent().parent().parent().find('.questionName').empty();
                    var thisValue = $(this).val();
                    // var beforeVal =  thisValue.val();
                    $(this).parent().parent().parent().find('.questionName').text(thisValue);
                });
            }
        });
    ...
    this.addNew = function(pos, method){
        ...
        if(val) {
            _new = mw.tools.cloneObject(JSON.parse(JSON.stringify([])));

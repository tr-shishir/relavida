...
mw._initHandles = {
    ...
    modules: function () {
        var handlesModulesButtons = [
            {
                ...
                icon: 'mdi-pencil',
                action: function () {
                    sessionStorage.removeItem('managetabsesstion');
            ...
            menu: [
                ...
                },
                {
                    ...
                    action: function () {
                        ...
                        // single_layout_copy_page_id =
                        // single_layout_copy_module_id = mw._activeModuleOver.id;
                        // single_layout_copy_field_name = parent.getAttribute('field');
                        // console.log(single_layout_copy_page_id,single_layout_copy_module_id,single_layout_copy_field_name);
                        sessionStorage.setItem("single_layout_copy_page_id",$("#page_id_for_layout_copy").val());
                        sessionStorage.setItem("single_layout_copy_module_id", mw._activeModuleOver.id);
                        sessionStorage.setItem("single_layout_copy_field_name", parent.getAttribute('field'));
                        mw.handleModule.hide();
                    }
                },
                {
                    title: mw.lang('Paste Up'),
                    icon: 'mdi mdi-content-duplicate',
                    className:'mw_handle_module_paste_up',
                    action: function () {
                        var parent = mw.tools.firstParentWithClass(mw._activeModuleOver, 'edit');
                        single_layout_paste_page_id = $("#page_id_for_layout_copy").val();
                        single_layout_paste_module_id = mw._activeModuleOver.id;
                        single_layout_paste_field_name = parent.getAttribute('field');

                        single_layout_copy_page_id = sessionStorage.getItem("single_layout_copy_page_id");
                        single_layout_copy_module_id = sessionStorage.getItem("single_layout_copy_module_id")
                        single_layout_copy_field_name = sessionStorage.getItem("single_layout_copy_field_name")
                        $.ajax({
                            type: "POST",
                            url: "api/v1/single_layout_copy",
                            data:{paste_position: 'up', single_layout_copy_page_id : single_layout_copy_page_id, single_layout_copy_module_id:  single_layout_copy_module_id, single_layout_copy_field_name: single_layout_copy_field_name,single_layout_paste_page_id : single_layout_paste_page_id, single_layout_paste_module_id:  single_layout_paste_module_id, single_layout_paste_field_name: single_layout_paste_field_name},
                            success: function(response) {
                                console.log(response.message);
                                $.get(mw.settings.api_url + "clearcache", {}, function () {
                                    mw.notification.success("The layout has been pasted successfully.");
                                    location.reload();
                                });
                            }
                        });
                        mw.handleModule.hide();
                        sessionStorage.removeItem("single_layout_copy_page_id");
                        sessionStorage.removeItem("single_layout_copy_module_id");
                        sessionStorage.removeItem("single_layout_copy_field_name");
                    }
                },
                {
                    title: mw.lang('Paste Down'),
                    icon: 'mdi mdi-content-duplicate',
                    className:'mw_handle_module_paste_down',
                    action: function () {
                        var parent = mw.tools.firstParentWithClass(mw._activeModuleOver, 'edit');
                        single_layout_paste_page_id = $("#page_id_for_layout_copy").val();
                        single_layout_paste_module_id = mw._activeModuleOver.id;
                        ...
                        $.ajax({
                            type: "POST",
                            url: "api/v1/single_layout_copy",
                            data:{ paste_position: 'down',single_layout_copy_page_id : single_layout_copy_page_id, single_layout_copy_module_id:  single_layout_copy_module_id, single_layout_copy_field_name: single_layout_copy_field_name,single_layout_paste_page_id : single_layout_paste_page_id, single_layout_paste_module_id:  single_layout_paste_module_id, single_layout_paste_field_name: single_layout_paste_field_name},
                            success: function(response) {
                                console.log(response.message);
                                $.get(mw.settings.api_url + "clearcache", {}, function () {
                                    mw.notification.success("The layout has been pasted successfully.");
        ...
        var positionModuleHandle = function(e, pelement, handle, event){
            ...
            mw.$('.mw_handle_module_paste_up').hide();
            mw.$('.mw_handle_module_paste_down').hide();
            if(isLayout){
                if(sessionStorage.getItem("single_layout_copy_module_id")){
                    mw.$('.mw_handle_module_paste_up').show();
                    mw.$('.mw_handle_module_paste_down').show();
                }else{
        ...
        mw.on('moduleOver', function (e, pelement, event) {
            ...
            mw.$('.module', pelement).each(function () {
                ...
                var btnid = 'btn-'+this.id;
                var btntextval = $("#"+btnid).text();

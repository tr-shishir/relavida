...
<?php if (is_array($data) and !empty($data)):
    if(isset($params['no_toolbar']) && $params['no_toolbar'] == true){
        $colCount = (get_option('data-limit',$params['id']) != "") ? get_option('data-limit',$params['id']) : 6;
    }

    if(isset($colCount)){
        $colCount = (get_option('data-limit',$params['id']) != "") ? get_option('data-limit',$params['id']) : 6;
        if(get_option('content_limit_for_live_edit_module','content_limit_for_live_edit_module')){
            if(get_option('content_limit_for_live_edit_module','content_limit_for_live_edit_module') < count($data)){
                $contentLimit = get_option('content_limit_for_live_edit_module','content_limit_for_live_edit_module');
            }else{
                $contentLimit = count($data);
            }
        }else{
            $contentLimit = count($data);
        }
            ...
            <?php if (is_array($data)):
                ...
                <?php foreach ($data as $key => $item):
                    ...
                    <?php if (isset($item['id'])):
                        ...
                        <div class="card card-product-holder mb-2 post-has-image-<?php print($pic == true ? 'true' : 'false'); ?> manage-post-item-type-<?php print $item['content_type']; ?> manage-post-item <?php ($item['is_rss'] == 1) ? print('rss_post') : print('own_post') ?> manage-post-item-<?php print($item['id']) ?> <?php print $pub_class ?>">
                            <div class="card-body">
                                <div class="row align-items-center flex-lg-nowrap">
                                    ...
                                    <div class="col item-title manage-post-item-col-3 manage-post-main">
                                        ...
                                        <div class="manage-post-item-links">
                                            ...
                                            <?php endif; ?>
                                            <?php if ($item['is_rss'] == 1) : ?>
                                                <button type="button" style="color: #ff4f52 !important;" class="btn  btn-info btn-sm" disabled><?php _e('rss'); ?></button>
                                            <?php endif; ?>

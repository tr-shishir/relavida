...
<script type="text/javascript">
    ...
    delete_selected_posts = function () {
        mw.tools.confirm("<?php _ejs("Are you sure you want to delete the selected posts"); ?>?", function () {
            var master = mwd.getElementById('<?php print $params['id']; ?>');
            var arr = mw.check.collectChecked(master);
			$.post("<?=api_url('delete_products_info')?>", {
                id:arr
            }).then((res, err) => {
            console.log(res, err);
            });
...
<div class="card style-1 mb-3 <?php if (isset($params['no_toolbar']) AND $params['no_toolbar']): ?>card-in-live-edit<?php endif; ?>">
    ...
    <div class="card-body pt-3">
        ...
        if (isset($params['data-page-number'])) {
            ...
            $numactive = 1;
            ?>
            <?php  if($params['data-id'] == "pages_edit_container"): ?>
                <script>
                    $(document).ready(function() {
                        mw.url.windowHashParam('<?php print $paging_param ?>','<?php print  $numactive ?>');return false;
                    });
                </script>
            <?php endif; ?>
            <?php
        ...
        if(isset($params['no_page_edit']) ){
            if( isset($params['content_type']) && $params['content_type'] == "product" ){

                ?>
                <div class="show-more">
                    <a href="<?=url("/admin/view:shop/action:products#pg=1")?>" target="_blank">Show More --></a>
                </div>
                <?php
            }elseif( isset($params['content_type']) && $params['content_type'] == "post" ){

                ?>
                <div class="show-more">
                    <a href="<?=url("/admin/view:content/action:posts#pg=1")?>" target="_blank">Show More --></a>
                </div>
                <?php
                }
        ...
        if (isset($paging_links) and is_array($paging_links)): ?>
            <ul class="mw-paging">
                <?php $i = 1; ?>
                <?php if(!($numactive == $pages_count) ): ?>
                    <li class="page-item">
                        <a class="page-link page-<?php print $pages_count; ?>" href="#<?php print $paging_param ?>=<?php print $pages_count ?>" onclick="mw.url.windowHashParam('<?php print $paging_param ?>','<?php print $pages_count ?>');return false;"><?php _e('Return'); ?></a>
                    </li>
                <?php endif; ?>
                <?php foreach ($paging_links as $item): ?>

                    <?php if($i == $numactive-3 or $i == $numactive+3 or $i == $numactive-2 or $i == $numactive+2 or $i == $numactive-1 or $i == $numactive+1 or $i == $numactive): ?>
                ...
                <?php if(!($numactive == $pages_count) ): ?>
                    <li class="page-item ">
                        <a class="page-link page-<?php print $numactive+1; ?>" href="#<?php print $paging_param ?>=<?php print $numactive+1 ?>" onclick="mw.url.windowHashParam('<?php print $paging_param ?>','<?php print $numactive+1 ?>');return false;"><?php _e('Continue'); ?></a>
                    </li>
                <?php endif; ?>
            </ul>
            <?php endif;

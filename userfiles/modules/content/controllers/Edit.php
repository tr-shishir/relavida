       ...
       // dd($params);
        ...
        if ($recommended_parent == false and intval($data['id']) == 0 and intval($data['parent']) == 0) {
            ...
            if (isset($params['subtype']) and $params['subtype'] == 'post') {
                ...
                if(isset($GLOBALS['blog_data'][0]['id'])){
                    $parent_content_params['id'] = $GLOBALS['blog_data'][0]['id'];
                }
                $parent_content = $this->app->content_manager->get($parent_content_params);
...
//var_dump($data);
        ...
        if($data['content_type'] == 'product'){
            $product_details = \DB::table('product_details')->select('suplier','tax_rate','tax_type', 'digital_opt', 'd_P_download_link','download_limit')->where('rel_id',$data)->first();

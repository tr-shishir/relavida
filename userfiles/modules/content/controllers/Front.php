    ...
    function index($params, $config)
    {
        ...
        if(CATEGORY_ID == false){
            ...
            if(get_option('data-category-id', $params['id'])){
                // $post_params['parent'] = $GLOBALS['shop_data'][0]['id'];
                $post_params['category'] = get_option('data-category-id', $params['id']);
            }
            if ($posts_parent_category == '') {
                $posts_parent_category = false;
            }
        }else{
            $posts_parent_category = $posts_parent_category_cfg = false;
        }
...
//            }
        ...
        if (@$_REQUEST['category_list'] == 'ean') {
            $ean = $_REQUEST['keywords'];
            //  dump($params);
            $post_params['__query_get_only_in_stock'] = function ($query) use ($ean){
                $query->where('content.ean','=',$ean);
            };

            unset($post_params['keyword']);
            unset($post_params['data-keyword']);

        }

        if (@$_REQUEST['category_list'] == 'sku') {
            $sku = $_REQUEST['keywords'];
            //  dump($params);
            $params['__query_get_only_in_stock'] = function ($query) use ($sku){
                $query->whereIn('content.id', function ($subQuery) use ($sku) {
                    $subQuery->select('content_data.rel_id');
                    $subQuery->from('content_data');
                    $subQuery->where('content_data.field_name', '=', 'sku');
                    $subQuery->where('content_data.field_value', '=', $sku);
                });
            };
            unset($post_params['keyword']);
            unset($post_params['data-keyword']);
        ...
        if($post_params['content_type'] == 'product'){
            $params['__query_get_only_in_stock'] = function ($query){
                $query->join('products','products.content_id','content.id');
                $query->where('products.category_hide','=',0);
            };

        }

        if(isset($post_params['type']) && $post_params['type'] == 'shop/thank_you'){
            $thankProduct = DB::table('thank_you_pages')->get();
            if(isset($thankProduct)){
                foreach( $thankProduct as $tp){
                    if(get_content("id=".$tp->product_id) == false){
                        DB::table('thank_you_pages')->where('product_id',$tp->product_id)->delete();
                    }else{
                        $content[] = get_content("id=".$tp->product_id)[0];
                    }
                }
            }
        }else{

            if ($post_params['content_type'] == 'post') {
                $blog_post_options = get_option('rss_option', 'rss_data');
                if (isset($blog_post_options) && !empty($blog_post_options)) {
                    if ($blog_post_options == 0 || $blog_post_options == false) {
                        $post_params = array_merge($post_params, ['is_rss' => 0]);
                        $content = get_content($post_params);
                    } elseif ($blog_post_options == 1) {
                        $content = get_content($post_params);
                    } elseif ($blog_post_options == 2) {
                ...
                $post_params['category_hide'] = true;
                $content = get_content($post_params);
            }
        }
        ...
        if (!empty($content)) {
            $normal_procedure = true;
            if($post_params['content_type'] == 'product'){
                if($params['content_type'] == 'checkout_bumbs' || $params['content_type'] == 'thank_you' || $params['content_type'] == 'thank_you_tag'){
            ...
            if($post_params['content_type'] == 'post'){
                if(isset(mw()->template->get_config()['optimized_blog_module']) && mw()->template->get_config()['optimized_blog_module'] =='yes'){
                    $blogs_id = null;
                    $data = null;
                    // dump($content);
                    $blogs = collect($content);
                    $blogs_id = $blogs->pluck('id')->toArray();
                    $data = DB::table('blogs')
                    ->whereIn('content_id',$blogs_id)
                    ->orderByRaw(DB::raw('FIELD(content_id, '.implode(', ', $blogs_id).')'))
                    ->get()
                    ->toArray();
                    $normal_procedure = false;
                }
            ...
            if($normal_procedure){
                foreach ($content as $item) {
                    ...
                    $iu = get_picture($item['id'], $for = 'post', $full = false) ?? false;
                    ...
                    if ($item['image'] != false) {
                        $item['tn_image'] = (isset($item['media'][0]['resize_image']))?$item['media'][0]['resize_image']:thumbnail($item['image'], $tn[0], $tn[1]);
                    ...
                    if($item['content_type'] == 'product'){
                        $product_details = \DB::table('product_details')->select('suplier','tax_rate','tax_type','offer_options','basic_price')->where('rel_id',$item['id'])->first();
                        $product_details = (array)$product_details;
                        if(@$product_details['tax_type'] == 2){
                            $taxRate = (is_logged()) ? (int)taxRateCountry(user_id(),$item['id']) : taxRate($item['id']);
                            $product_details['tax_rate'] = $taxRate;
                        }
                        if(!empty($product_details)){
                            $item = array_merge($item,$product_details);
                        }
                    }

...
<script type="text/javascript">
    mw.menu_add_new = function () {
        ...
        $.post("<?php echo route('api.menu.create'); ?>", obj, function (data) {
            ...
            if(!data.error && mw.notification != undefined){
                mw.notification.success(mw.data.settingsSaved);
            }else{
    ...
    mw.menu_delete = function ($id) {
        var data = {};
        data.id = $id
        mw.confirm('<?php _e('Are you sure you want to delete this menu?'); ?>', function(){
...
$menu_id = get_menus('one=1&title=' . $menu_name);
if ($menu_id == false) {
    if(isset($menus[0]['title'])){
        $default_menu_name = $menus[0]['title'];
    }else{
        $default_menu_name = 'header_menu';
    }
    $active_menu = $menu_name = $default_menu_name;
    $menu_id = get_menus('one=1&title=' . $menu_name);
}
        ...
        <div class="form-group">
            ...
            <select id="menu_selector_<?php print $params['id'] ?>" name="menu_name" class="mw_option_field selectpicker" data-width="100%" data-size="5" onchange="mw.menu_edit_items(this.value, '#items_list_<?php print $rand ?>');" onblur="mw.menu_edit_items(this.value, '#items_list_<?php print $rand ?>');">
                <?php foreach ($menus as $item): ?>
                    ...
                    ?>
                    <option <?php if ($menu_name == $item['title'] or $menu_data == $item['id']): ?><?php $active_menu = $item['title'] ?>selected<?php endif; ?> value="<?php print $item['title'] ?>"><?php _e(ucwords(str_replace('_', ' ', $item['title']))) ?></option>
                <?php endforeach; ?>
            </select>
        </div>
    <?php endif; ?>

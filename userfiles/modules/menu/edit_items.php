...
<?php if (isset($data) and $data != false): ?>
    <div class="form-group">
        <label class="control-label"><strong><?php _e(strtoupper(str_replace('_', ' ', $menu_title))); ?> <?php _e("structure"); ?> <?php _e("structure"); ?></label>
...
<?php if ($id != 0): ?>
    <div class="d-flex justify-content-between align-items-center mt-3">
        <p class="m-0 font-weight-bold"><?php _e(strtoupper(str_replace('_', ' ', $menu_title)).' '.'Selected'); ?></p>
        <a href="javascript: mw.menu_delete('<?php print $id; ?>');" class="btn btn-outline-danger btn-sm"><strong><?php _e(strtoupper(str_replace('_', ' ', $menu_title)).' '.'Delete'); ?></strong></a>
        <a data-toggle="modal" data-target="#menuModal" class="btn btn-outline-primary btn-sm"><strong><?php _e(strtoupper(str_replace('_', ' ', $menu_title)).' '.'Edit'); ?></strong></a>
    </div>
    <style type="text/css">
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          <h4 class="modal-title"><?php _e('Menu Name'); ?></h4>
          <button type="button" class="close" data-dismiss="modal">&times;</button>
        </div>
        <div class="modal-body">
            <form action="<?php print(url('api/v1/menu-edit')); ?>" method="POST">
                
                <input type="hidden" name="_token" value="<?php print(csrf_token()) ?>">
                <div class="form-group">
                    <label for="usr"><?php _e('Menu Name'); ?></label>
                    <input type="text" class="form-control" name="menu_title" value="<?php print str_replace('_', ' ', $menu_title); ?>">

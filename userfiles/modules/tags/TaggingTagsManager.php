...
function tagging_tag_autocomplete($params)
{
    ...
    if ($content_id_get == 4) {
        ...
        if ($content_id_get == 1) {
            $type = 'product';
        } elseif ($content_id_get == 2) {
            $type = 'page';
        } elseif ($content_id_get == 3) {
            $type = 'post';
        } elseif ($content_id_get != null) {
            $content = DB::table('content')->where('id', $content_id_get)->first();
            $type = $content->content_type;
        }

        $all_content = DB::table('content')->where('content_type', $type)->get('id');
        $founded_tags = [];
        ...
        if ($tagging_tags) {
            foreach ($tagging_tags as $tagging_tag) {
                $tag_value_id = DB::table('tagging_tagged')->where('tag_name', $tagging_tag['name'])->first();

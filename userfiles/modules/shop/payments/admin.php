...
<script type="text/javascript">
    ...
    function testStatus(data){
        if(data == 'n'){
            $('#test_off').css('display', '');
            $('#test_on').css('display', 'none');
        } else{
            $('#test_off').css('display', 'none');
            $('#test_on').css('display', '');
        }
    }
</script>
...
<div class="card bg-none style-1 mb-0">
                            ...
                            <div class="card-body py-3">
                                <div class="row d-flex align-items-center">
                                    ...
                                    <div class="col text-right">
                                        <?php if(!('Test Payment' == $payment_module['name'])): ?>
                                ...
                                <template class="js-modal-content" style="display: none;">
                                    <h5 class="mb-0"><?php _e('Enter your '.$payment_module['name'].' data here'); ?></h5>
                                    <small class="text-muted mb-3 d-block"><?php _e('Without this data, your customer cannot pay via '.$payment_module['name']); ?></small>
    ...
    <?php
    if("admin/view:shop/action:options/"  == url_path()):
    ?>
    <div class="card bg-none style-1 mb-0">
        <div class="card-header px-0">
            <h5>
                <i class="mdi mdi-cash-usd-outline text-primary mr-3"></i> <strong><?php _e("Subscription Payment methods"); ?></strong>
            </h5>
        <div></div>
    </div>

    <div class="card-body pt-3 px-0">
        <p class="text-muted"><?php _e("Enable and set up the payment method your customers will use to pay for Subscription product"); ?></p>

        <div id="db-payment-methods">
            <module type="shop/payments/gateways/subscription/adminSetting" />
        </div>
    </div>
    <?php
    endif;

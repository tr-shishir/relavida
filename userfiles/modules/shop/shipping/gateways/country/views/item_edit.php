...
<script type="text/javascript">
    ...
    SaveShipping = function (form, dataType) {
        ...
        mw.form.post($(form), '<?php print $config['module_api']; ?>/shipping_to_country/save', function () {
            ...
            mw.notification.success("<?php _ejs("Shipping changes are saved"); ?>");
            mw.reload_module('shop/shipping/gateways/country/add_country')
        ...
        <div class="<?php if ($item['is_active'] == 1 AND $new == false): ?>box-enabled-<?php elseif ($item['is_active'] == 0): ?>box-disabled-<?php endif; ?> <?php if ($new == false): ?>toggle-item closed-fields<?php endif; ?>">
            ...
            <div class="form-group">
                <label class="control-label">
                    <?php if ($new == true): ?>
                        <?php _e("Select a new country"); ?>
                    <?php else : ?>
                        <?php _e("Shipping to country"); ?>
                    <?php endif; ?>
                </label>

                <small class="text-muted d-block mb-2"><?php _e("Please, select a country to add"); ?></small>
                ...
                <select name="shipping_country" class="selectpicker" data-width="100%" data-size="5" onchange="SaveShippingForm();" <?php if (isset($item['id']) AND $item['id'] != 0): ?>disabled<?php endif; ?>>
                    ...
                    <?php if (is_array($countries)) : ?>
                        <?php foreach ($countries as $item1): ?>
                            ...
                            endforeach;
                            ?>
                            <option value="<?php print $item1 ?>" <?php if (isset($item['shipping_country']) and $item1 == $item['shipping_country']): ?> selected="selected" <?php else : ?><?php print $disabled ?><?php endif; ?> ><?php ($item1=="Worldwide") ?  _e("All Listed Country") : print $item1 ; ?></option>
                        <?php endforeach; ?>
                    <?php endif; ?>
                </select>
            </div>
            ...
            <div class="js-shipping-item-edit-needs-id">
                            ...
                            <div class="form-group">
                                ...
                                <div class="custom-control custom-radio mb-4">
                                    ...
                                    <label class="custom-control-label" for="shipping_type2"><?php _e("Dimensions or Weight"); ?>
                                        <small class="text-muted d-block"><?php _e('Set a flexible shipping price depending on a products dimensions or weight'); ?></small>
                                    </label>
                                </div>
                    ...
                    <div class="col-5">
                        <div class="form-group">
                            ...
                            <div class="input-group input-group-sm mb-3">
                                ...
                                <input class="form-control" type="text" oninput="mw.form.typeNumber(this);execute_function(this.id);"   placeholder="0" id="shipping_cost" name="shipping_cost" value="<?php print mw()->shop_manager->shipping_currency_format($item['shipping_cost']); ?>"/>
                            </div>
                        </div>
                ...
                <?php if(isset($item['shipping_cost']) && $item['shipping_cost']>0){ ?>
                <hr class="thin"/>
                ...
                <div class="form-group">
                        <label class="control-label"><?php _e("Minimum shopping cart value for free or reduced shipping"); ?></label>
                        <br>
                        ...
                        <div class="input-group input-group-sm mb-2 mt-2">
                            ...
                            <input class="form-control" type="text" oninput="mw.form.typeNumber(this);execute_function(this.id);" id="shipping_cost_above" name="shipping_cost_above" value="<?php print mw()->shop_manager->shipping_currency_format($item['shipping_cost_above']); ?>"   placeholder="">
                        </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="input-group input-group-sm mb-2">
                            ...
                            <input class="form-control shipping-price-field" type="text" oninput="mw.form.typeNumber(this);execute_function(this.id);"  id="shipping_cost_max" name="shipping_cost_max" value="<?php print mw()->shop_manager->shipping_currency_format($item['shipping_cost_max']); ?>"  placeholder=""/>
                        </div>

...
<?php if ($cart_items and is_array($cart_items)) : ?>
    ...
    <div class="table-responsive">
        <table class="table vertical-align-middle table-header-no-border table-primary-hover" id="order-information-table">
            ...
            <tr>
                <th><?php _e("Bild"); ?></th>
                <th><?php _e("Produktname"); ?></th>
                <th><?php _e("SKU"); ?></th>
                <th><?php _e("Preis"); ?></th>
                <th><?php _e("Servicepreis"); ?></th>
                <th class="text-center"><?php _e("Menge"); ?></th>
                <th><?php _e("Gesamt"); ?></th>
                <th></th>
            </tr>
            </thead>
            ...
            foreach ($cart_items as $item) : ?>
                <?php
                $index++;
                $orderedUpselling = DB::table("order_with_upselling")->where('product_id', $item['rel_id'] )->where('user_id',user_id())->where('order_id',$ord['id'])->get();
                // dump($orderedUpselling);
                $servicePrice = 0;
                if($orderedUpselling->count()){
                    foreach($orderedUpselling as $orderValue){
                        $serviceprice = DB::table("product_upselling")->where('id', $orderValue->service_id )->get();
                        $servicePrice = $servicePrice + $serviceprice[0]->servicePrice;
                    }
                    $taxam = 0; $tax= mw()->tax_manager->get();
                    !empty($tax) ? $taxam = $tax['0']['rate'] : $taxam = 0 ;
                    $servicePrice = $servicePrice + ($taxam*$servicePrice)/100;
                    $product_price = roundPrice($item['price'] + ($taxam*$item['price'])/100);
                    $item_total = floatval($item['qty']) * ($product_price +  $servicePrice);
                }
                else{
                    $taxam = 0; $tax= mw()->tax_manager->get();
                    !empty($tax) ? $taxam = $tax['0']['rate'] : $taxam = 0 ;
                    $product_price = roundPrice($item['price'] + ($taxam*$item['price'])/100);
                    $item_total = floatval($item['qty']) * $product_price;
                }
                ...
                <tr data-index="<?php print $index; ?>" class="mw-order-item mw-order-item-<?php print $item['id'] ?> mw-order-item-index-<?php print $index; ?>">
                    ...
                    <td class="mw-order-item-amount"><?php print  currency_format( $product_price, $ord['currency']); ?></td>
                    <td class="mw-order-item-service-amount"><?php print  currency_format( $servicePrice, $ord['currency']); ?></td>
    ...
    <div class="row">
        <div class="info-table col-md-8 col-lg-6 col-xl-5 ml-auto">
            ...
            <div class="row">
                ...
                <?php 
                if (isset($ord['taxes_amount']) and $ord['taxes_amount'] != false){
                    $subtotal = $subtotal - $ord['taxes_amount'];
                }

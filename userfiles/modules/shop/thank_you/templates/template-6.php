...
?>
<?php if (!empty($data)): ?>
    <div class="edit" field="thank_you_page_top_six" rel="content">
    <module type="text"/>
    ...
    <div class="shop-products related-products thank-you-row">
        <?php foreach ($data as $item):  ?>
           <?php
           $temp = DB::table("thank_you_pages")->where('template_name','6')->where('product_id',$item['id'])->get();
           ?>
            <?php if($temp->count()): ?>
                        ...
                        <a href="<?php print $item['link'] ?>">
                            <div class="description">
                               ...
                               <div class="thank-you-cartButton">
                                    ...
                                    <?php if($GLOBALS['6'] == $activeThemeQuantity->count()): ?>
                                        <a data-toggle="modal" data-target="#thankYou-redirect" class="btn btn-primary thank-you-cartbtn" onclick="mw.cart.add('.shop-products .item-<?php print $item['id'] ?>');  theme6(); totalPrice();"><span class="material-icons">shopping_basket</span>
                                        JETZT KAUFEN
                                        </a>
                                    <?php else: ?>
                                        <a class="btn btn-primary thank-you-cartbtn" onclick="mw.cart.add('.shop-products .item-<?php print $item['id'] ?>');  theme6();"><span class="material-icons">shopping_basket</span>
                                        JETZT KAUFEN
                                        </a>
                                    <?php endif; ?>
                               </div>
    ...
    <div class="edit" field="thank_you_page_bottom_six" rel="content">
    <module type="text"/>
...
function theme6(){
        ...
        var item={name: '<?php print  $title6 ?>', price: '<?php print  $price6 ?>', sprice: '<?php print  currency_format($price6) ?>'};

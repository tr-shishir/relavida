...
<style>
    ...
    .mw-file-browser-header .dropdown,
    .mw-browser-uploader-path {
    ...
    .mw-browser-list-item .mw-file-item-check {
        position: absolute;
        top: 12px;
        left: 5px;
        width: calc(100% - 7px);
    ...
    .search-index-disable-eachfile {
        position: relative;
        padding-left: 17px;
    }

    .search-index-disable-eachfile input {
        position: absolute;
        left: 0px;
        top: 4px;
    }

    .mw-browser-list-item .mw-file-item-check input {
    }
</style>
                            ...
                            <div class="mw-file-browser">
                                <div class="mw-file-browser-header">
                                    <div>
                                        ...
                                        <a href="javascript:;" onclick="back()" class="btn btn-outline-primary icon-left btn-sm">
                                            <i class="mdi mdi-keyboard-backspace" style="margin-top: -4px;"></i>
                                            <?php _e("Back"); ?>
                                        </a>

                                        <!-- <div class="search-index-disable">
                                            <?php if (Config::get('custom.disableGoggleIndex') == 1) : ?>
                                                <input type="checkbox" name="disableGoggleIndex" id="disableGoggleIndex" checked>
                                            <?php else : ?>
                                                <input type="checkbox" name="disableGoggleIndex" id="disableGoggleIndex">
                                            <?php endif; ?>
                                            <label for="disableGoggleIndex">Disallow for Google Index</label>
                                        </div> -->
                                        <script>
                                            $("#disableGoggleIndex").on("click", function() {
                                                if ($('input[name="disableGoggleIndex"]').is(':checked')) {
                                                    //    alert("Checked");
                                                    $.ajax({
                                                        type: "POST",
                                                        url: "<?= api_url('disableGoggleIndex_check') ?>",
                                                        data: {
                                                            check_index: 1
                                                        }
                                                    });
                                ...
                                <div id="mw-browser-list-holder">
                                        ...
                                        <ul class="<?php print $browser_list_holder_class ?>">
                                            <?php foreach ($data['dirs'] as $item) : ?>
                                                ...
                                                <li class="mw-browser-list-folder mw-browser-list-create">
                                                    ...
                                                    <a title="<?php print basename($item); ?>" href="#path=<?php print urlencode($dir_link); ?>">
                                                        <span class="as-image">
                                                            <span class="mdi mdi-folder"></span>
                                                            <span><?php print basename($item); ?></span>
                                                        </span>
                                                    </a>
                                                    <span class="mw-file-item-delete" onclick="deleteItem('<?php print urlencode($item); ?>', '<?php print basename($item); ?>');"></span>
                                                </li>
                                            <?php endforeach; ?>
                                        </ul>
                                    <?php else : ?>
                                    ...
                                    <?php if (isset($data['files'])) : ?>
                                        <ul class="<?php print $browser_list_holder_class ?>">
                                            <?php foreach ($data['files'] as $key => $item) :
                                                ?>
                                                <li>
                                                    <div class="mw-browser-list-item">
                                                        <a title="<?php print basename($item); ?>" class="mw-browser-list-file mw-browser-list-<?php print substr(strrchr($item, '.'), 1); ?>" href="<?php print mw()->url_manager->link_to_file($item) ?>" onclick="select(this, '<?php print mw()->url_manager->link_to_file($item) ?>'); return false;">
                                                            <?php $ext = strtolower(get_file_extension($item)); ?>
                                                            <?php if ($ext == 'jpg' or $ext == 'png' or $ext == 'gif' or $ext == 'jpeg' or $ext == 'bmp' or $ext == 'webp') : ?>
                                                                <span data-src="<?php print thumbnail(mw()->url_manager->link_to_file($item), $tn_size, $tn_size, true); ?>" class="<?php print basename($item) ?> as-image image-item-not-ready"></span>
                                                            <?php else : ?>
                                                                <div class="mw-fileico mw-fileico-<?php print $ext; ?>">
                                                                    <span><?php print $ext; ?></span>
                                                                </div>
                                                            <?php endif; ?>
                                                        ...
                                                        <div class="mw-file-item-check">
                                                            <label class="mw-ui-check pull-left">
                                                                <input type="checkbox" oninput="gchecked()" name="fileitem" id="v<?php print $rand; ?>" value="<?php print $item; ?>" />
                                                                <span></span>
                                                            </label>
                                                            <span class="mw-file-item-delete" onclick="deleteItem(mwd.getElementById('v<?php print $rand; ?>').value);"></span>
                                                        </div>
                                                        <div class="search-index-disable-eachfile">
                                                            <input type="checkbox" class="disableGoggleIndex" name="fileImgid[]" data-id="<?=$key?>" data-filename="<?php print basename($item); ?>">
                                                            <label for="">Disallow for Google Index</label>
                                                        </div>
                                        ...
                                        <script>
                                            ...
                                            $(".disableGoggleIndex").on("click",function(){
                                                ...
                                                var imgData = []
                                                $("input[name='fileImgid[]']:checked").each(function ()
                                                {
                                                    imgData.push($(this).data('filename'));
                                                });
                                                //console.log(imgData);
                                                ...
                                                $.post("<?= url('/') ?>/api/v1/index_prevent", { filename:imgData}, (res) => {
                                                    if(res.success){
                                            ...
                                            // Get Checked value Of Dissallowed Imaga
                                            $(document).ready(function() {
                                                $.post("<?= url('/') ?>/api/v1/getindex_prevent", {}, (res) => {
                                                    //console.log(res[0]);
                                                    res.forEach(function(resImg){
                                                        // console.log(resImg);
                                                        $('[data-filename="'+resImg+'"]').prop("checked", true);
                                                        // console.log('\\userfiles\\media\\default\\'+resImg);
                                                    });
                                                    var imgFolderLocation = 'Disallow: \\userfiles\\media\\default\\';
                                                    var imgFullUrl = res.map(el => imgFolderLocation + el);

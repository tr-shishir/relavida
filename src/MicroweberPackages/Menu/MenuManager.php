...
class MenuManager
{
    ...
    {
        try {
            $validator = Validator::make($data_to_save, [
                'title' => 'required|unique:menus|max:300'
            ],[
...
//        }
        if (isset($url_menu) && !empty($url_menu)) {
            $data_to_save['url'] = $url_menu;
        }
...
//        }
        ...
        if (!isset($link) or $link == false) {
            $link = '<a itemprop="url" data-item-id="{id}" class="menu_element_link {active_class} {exteded_classes} {nest_level} {a_class} {a_submenu_class}"  {target_attribute} href="{url}" {tooltip}>{title}</a>';
        }
        ...
        foreach ($q as $item) {
            ...
            if(isset($item['description']) and !empty($item['description'])){
         ...
         /*   $override = $this->app->event_manager->trigger('menu.after.get_item', $item);
            ...
            if ($title != '') {
               ...
               // $sub_menu_params['count'] = true;
                ...
                $menu_link = str_replace('{tooltip}', $tooltip, $menu_link);
                $to_print .= $menu_link;
...
*/
    ...
    public function menu_delete($id = false)
    {
        ...
        $this->app->database_manager->delete_by_id($table, trim($id), $field_name = 'parent_id');
        $this->app->cache_manager->delete('menus');

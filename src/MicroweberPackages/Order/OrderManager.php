...
//            }
//        }
		$tax_get = DB::table('cart_orders')->where('id', $ord)->first();
		
		if($sub != null){
            // $status = DB::table('cart')->where('rel_id', $sub->product_id)->update(['order_completed' => 1, 'order_id' => $ord]);
            DB::table($this->app->cart_manager->table_name())->whereRelId($sub->product_id)->whereOrderCompleted(0)->whereSessionId($sid)->update(['order_id' => $ord, 'order_completed' => 1]);
            if(isset($place_order['amount'])){
                DB::table('subscription_order_status')->where('product_id', $sub->product_id)->where('order_id', null)->where('user_id', $u_id)->update(['order_price' => $place_order['amount']]);
            }
            if(isset($tax_get)){
                DB::table('subscription_order_status')->where('product_id', $sub->product_id)->where('order_id', null)->where('user_id', $u_id)->update(['tax_amount' => $tax_get->taxes_amount]);
            }
            $status1 = DB::table('subscription_order_status')->where('product_id', $sub->product_id)->where('order_id', null)->where('user_id', $u_id)->update(['order_id' => $ord, 'order_status' => "unpaid", 'order_count' => 1, 'order_type' => "new"]);
		...
		googleAnalyticalOrder($tax_get->amount, $tax_get->currency, $tax_get->id);

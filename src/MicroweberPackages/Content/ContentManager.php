    ...
    public function paging($params)
    {
        ...
        if (is_array($data)) {
            ...
            if ($current_page_from_url != false) {
                $active_item = intval($current_page_from_url);
            }
            ...
            foreach ($data as $key => $value) {
                ...
                if($active_item == $key || ($active_item -1) == $key || ($active_item -2) == $key || ($active_item + 1) == $key || ($active_item + 2) == $key){
                    $ready_paging_number_links[] = [
                        'attributes' => [
                            'class' => $act_class,
                            'current' => $act_class,
...
//            }
    ...
    public function breadcrumb($params = false)
    {
        ...
        $breadcrumb_url = url_segment();
        $urth = end($breadcrumb_url);
        $cat = get_categories('url='.$urth);
        if(isset($cat) == false){
            $breadcrumb_cat = get_categories_for_content(CONTENT_ID);
            $cat = end($breadcrumb_cat);
        }

        if (defined('CATEGORY_ID') and CATEGORY_ID != false) {
            $cur_category = $cat['id'] ?? CATEGORY_ID;
        }
            ...
            if ($cur_category_data != false and isset($cur_category_data['id'])) {
                if($get_content && $get_content[0]['content_type']=='product'){
                    ...
                    if (!empty($cat_parents)) {
                        ...
                        $new_result= array_reverse($new_result);
...
//        if (env('SYNC_ENABLE') && isset($data['content_type']) && $data['content_type'] == 'product') {

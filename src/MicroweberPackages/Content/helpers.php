...
function get_content_by_id($params = false)
{
    ...
    if(@$posts['content_type'] == "post" && trim(strip_tags($posts['content']))){
        if($status == 1 or $posts['require_login'] == 1) {
            ...
            $posts['content'] = $get_content_with_html.'<br><span style="overflow: hidden;position: relative;display: block;"><a href="#" class="dtModalBtn" data-toggle="modal" data-target="#loginModal" style="float: right;background-color: #4592ff;color: #fff;padding: 15px 35px;border-radius: 5px;display: inline-block;text-decoration: none;">Einloggen und Bericht in voller Länge kostenfrei lesen</a></span>';
        }else{
            $posts['content'];
        }
        return $posts;

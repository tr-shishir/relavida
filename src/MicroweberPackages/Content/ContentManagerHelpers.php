...
use App\Models\Content as ProductContent;
...
class ContentManagerHelpers extends ContentManagerCrud
{
    ...
    public function delete($data)
    {
        ...
        if (isset($data['id'])) {
            $c_id = intval($data['id']);
            $del_ids[] = $c_id;
            $product = DB::table('products')->where('content_id',$c_id)->first();
            ...
            if ($to_trash == false) {
                // $product = DB::table('products')->where('content_id',$c_id)->first();
                if(isset($product) && !empty($product)){
                    DB::table('products')->where('content_id',$c_id)->delete();
                }
            ...
            foreach ($data['ids'] as $value) {
                ...
                if ($c_id) {
                    ...
                    if ($to_trash == false) {
                        ...
                        DB::table('categories_items')->where('rel_id',$c_id)->delete();
                        //permanently delete the blog
                        delete_blog_information($c_id);
                        $this->app->database_manager->delete_by_id('content', $c_id);
                    }
                }
            }
        ...
        if (!empty($del_ids)) {
            ...
            foreach ($del_ids as $value) {
                ...
                if ($c_id) {
                    ...
                    $content = ProductContent::find($c_id);
                    if ($to_untrash == true) {
                        ...
                        //restore the blog information
                        $blog_from_content_table = DB::table('content')->where('id',$c_id)->where('is_active',1)->first();
                        if($blog_from_content_table){
                            $blog_image_link = get_first_position_image_from_media($c_id);
                        ...
                        DB::table($this->tables['content'])->whereId($c_id)->update(['is_deleted' => 1, 'updated_at' => \Carbon::now()->toDateTimeString()]);
                        DB::table($this->tables['content'])->whereParent($c_id)->update(['is_deleted' => 1, 'updated_at' => \Carbon::now()->toDateTimeString()]);

                        if (isset($this->tables['categories'])) {
                            DB::table($this->tables['categories'])->whereRelId($c_id)->whereRelType('content')->update(['is_deleted' => 1, 'updated_at' => \Carbon::now()->toDateTimeString()]);
                        }
                        if(isset($product) && !empty($product)){
                            DB::table('products')->where('content_id',$c_id)->delete();
                        }
        ...
        //                    if (env('SYNC_ENABLE') && $content->content_type = 'product') {
                        ...
                        if ($save_global == false and $save_layout == false) {
                            if ($content_id) {
                                ...
                                if ($is_no_save != true and $is_draft == false) {
                                    ...
                                    if(isset($saved)){
                                        if(isset($ref_page) and isset($ref_page['content_type']) and $ref_page['content_type'] == 'post'){
                                            if(isset($post_data)){
                                                foreach($post_data as $single_item){
                                                    if(isset($single_item['attributes']['field']) and $single_item['attributes']['field']=='content' and isset($single_item['attributes']['rel']) and $single_item['attributes']['rel']=='content'){
                                                        DB::table('blogs')->where('content_id', $ref_page['id'])->update(['content' => $single_item['html']]);
                                                    }
                                                }
                                            }
                                        }

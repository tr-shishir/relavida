...
class ContentManagerCrud extends Crud
{
    ...
    public function get($params = false)
    {
        ...
        if (isset($params['keyword']) and !isset($params['search_in_fields'])) {
            ...
            $params['search_in_fields'] = array('title', 'content_body', 'url');
        }
        ...
        if (@$params['brand']) {
            $brand = $params['brand'];
            //  dump($params);
            $params['__query_get_only_in_stock_brand'] = function ($query) use ($brand){
                $query->where('content.brand','like','%'.$brand.'%');
            };
            unset($params['brand']);
        }
        if (@$params['ean']) {
            $ean = $params['ean'];
            $params['__query_get_only_in_stock_ean'] = function ($query) use ($ean){
        ...
        if (@$params['sku']) {
            $sku = $params['sku'];
            //  dump($params);
            $params['__query_get_only_in_stock_sku'] = function ($query) use ($sku){
        ...
        if(isset($params['category_hide'])){
            $params['__query_get_only_in_stock'] = function ($query){
                $query->whereIn('content.id', function ($subQuery) {
                    $subQuery->select('products.content_id');
                    $subQuery->from('products');
                    $subQuery->where('products.category_hide', '=', 0);
                });
            };
        }
         if (isset($params['filter-only-in-stocks'])) {
            $params['__query_get_only_in_stocks'] = function ($query){
                $query->whereIn('content.id', function ($subQuery)  {
                    $subQuery->select('content_data.rel_id');
                    $subQuery->from('content_data');

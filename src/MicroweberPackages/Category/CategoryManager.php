...
class CategoryManager
{
    ...
    public function save($data, $preserve_cache = false)
    {
        ...
        if (isset($data['url']) and trim($data['url']) != false) {
            ...
            if ($possible_slug) {
                ...
                if ($possible_slug != '') {
                    $check_cont_wth_slug = get_content('url='.$possible_slug);
        ...
        //        if (env('SYNC_ENABLE') && $data['rel_type'] == 'content') {
        if ($data['rel_type'] == 'content' && isset($shop_id) && ($shop_id['id'] == $data['rel_id'] || $data['rel_id'] == 0)) {
    ...
    public function delete($data)
    {
        ...
        $category = \App\Models\Category::find($c_id);
        ...
        //        if (env('SYNC_ENABLE') && $category->rel_type = 'content') {
        //        if ($category->rel_type = 'content' && isset($shop_id) && $shop_id['id'] == $category->rel_id) {
        if ($category->rel_type = 'content' && isset($shop_id) && ($shop_id['id'] == $category->rel_id || $category->rel_id == 0)) {
            //category delete
            SyncHistory::create([
                'sync_type' => SyncType::CATEGORY,

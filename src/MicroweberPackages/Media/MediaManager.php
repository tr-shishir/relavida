    ...
    public function get_picture($content_id, $for = 'content', $full = false)
    {
        ...
        if ($images != false and isset($images[0])) {
            if (isset($images[0]['filename']) and $full == false) {
                ...
                if(!remote_file_exists($img)){
                    $img = $this->app->format->replace_once('{SITE_URL}', $surl, $images[0]['webp_image']) ?? $this->app->format->replace_once('{SITE_URL}', $surl, $images[0]['filename']);
                }
                return $img;
            } else {
                return $images[0];
...
//d($params);
        ...
        $media_id = collect($data)->pluck('id')->toArray();
        $instagram_feed = DB::table('instagram_feed');
        if(count($media_id) > 1){
        ...
        if (!empty($data)) {
            ...
            foreach ($data as $item) {
                if(!empty($instagram_feed)){
                    $item['insta_details'] = $instagram_feed[$item['id']] ?? '';
                }
                ...
                if(isset($item) && !empty($item['webp_image']) && strpos( $_SERVER['HTTP_ACCEPT'], 'image/webp' ) !== false ){
                    $item['filename'] = $item['webp_image'];
                }else{
    ...
    public function thumbnail($src, $width = 200, $height = null, $crop = null)
    {
        ...
        if ($is_remote) {
            ...
            $resize_img = DB::table('media')->where('webp_image' , '{SITE_URL}'.$base_src)->first();
            if($width <= 800){
                $cache_path = @$resize_img->resize_image ?? $src;
            }else{
                $cache_path = $src;
            }

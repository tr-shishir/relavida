...
class JsCompileController extends Controller
{
    ...
    public function apijs_combined_cache(){
        //        if(file_exists(template_dir()template_dir().'assets/js/new.js')){
        //            return '';
        //        }
                $userfiles_dir = userfiles_path();
                $hash = md5(site_url());
        ...
        //            }
            ...
            {
               $url=site_url();
               return \Cache::remember($url.'-user-id-'.(@user_id()??99999), 600000, function () {
                    return $this->apijs_combined_cache();
               });
            }

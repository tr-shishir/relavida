...
class SitemapController extends Controller
{
   ...
   public function products()
   {
       ...
       $fp = fopen($generatedSiteMapFile, 'r');
       $total = Product::all();
       $total_item = count($total);
       if($total_item > 1000){
            $products = Product::paginate(1000);
            return view('sitemap::products', compact('products'));
       }
   ...
   public function pages()
   {
       ...
       if($updateSitemap) {
           if($this->isMutilangOn()) {
               ...
               $pagesData = $this->fetchNotMutilangPages();
           }
   ...
   private function fetchNotMutilangCategories()
   {
       ...
       foreach($categories as $cat) {
           ...
           $tmp = [
               ...
               'updated_at' => $cat->updated_at->format('Y-m-d'),
           ];
           if($tmp['original_link'] != null){
                if($shop == $tmp['original_link'] || $blog == $tmp['original_link']){}
                    else{
   ...
   private function fetchNotMutilangPages()
   {
       $pages = Page::all();
       $pagesLinksData = [];
   ...
   private function fetchNotMutilangProducts()
   {
       ...
       foreach($products as $prod) {
           $tmp = [
               ...
               'updated_at' => $prod->updated_at->format('Y-m-d'),
           ];
           if($tmp['original_link'] != null){
